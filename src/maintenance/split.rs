...
//! `Ready → Splitting → DrainingSplit →` completed, with both targets exposed
//! as `ReceivingSplit` and promoted to `Ready` at completion — composing the
...
//! - **Bounded work.** Each step commits at most one transition, one atomic
//!   target exposure, or one schema- and Backend-budget-bounded drain batch —
...
//!   training, and an already-exposed target always keeps its persisted
//!   centroid.
...
//!   partition, exposing a `Splitting` one, draining one batch
...
pub enum TargetExposure {
    ...
    /// centroids (by this or a previous committed attempt) and the source is
    /// `DrainingSplit`.
    Exposed {
        /// The left target.
        left: PartitionKey,
...
/// Trains and exposes both split targets of one `Splitting` source, advancing
/// it to `DrainingSplit` in the same transaction.
///
/// Training reads the complete source through one consistent snapshot while
/// the source is `Splitting`, outside any write transaction, via
/// [`training::train_split_centroids`]. One bounded transaction then
/// unique-creates both targets with the trained centroids, installs their
/// incoming Child Entries, and advances the source (ADR 0014). The first
/// committed exposure fixes both centroids forever: a competing attempt
/// conflicts on the update-protected source authority pair and retries onto
/// the persisted `DrainingSplit` state, so no worker ever overwrites a
/// published centroid. Centroids are routing models, not authority (exact
...
/// writes.
///
/// An exposure whose discovered parent cannot accept new children is abandoned
/// and retried from a fresh snapshot under the retry policy, so a later
/// attempt rediscovers the source's current parent. That outer retry
...
pub async fn expose_targets<B: Backend>(
    ...
    let (left, right) = match state {
        ...
        // Exposure and the DrainingSplit transition commit atomically, so a
        // DrainingSplit source is already fully exposed.
    ...
    loop {
        let outcome = writes::run_write_attempts(
            ...
            Operation::SplitFixup,
            |txn| {
                writes::boxed_step(topology::expose_split_targets(
                    txn,
                    tree_key,
                    source,
                    [(left, centroids.left()), (right, centroids.right())],
                    started_at_unix_millis,
                ))
            },
        )
        .await?;
        match outcome {
            // The parent rejected the new children: abandon and rediscover
            // from a fresh snapshot under the same bounded policy.
            topology::SplitExposure::ParentNotAccepting => {
                retry
                    .wait_or_exhaust(Operation::SplitFixup, &mut failed_attempts)
                    .await?;
            }
            topology::SplitExposure::Exposed | topology::SplitExposure::AlreadyExposed => {
                return Ok(TargetExposure::Exposed { left, right });
            }
            topology::SplitExposure::SourceAdvanced => {
                return Ok(TargetExposure::SourceAdvanced);
            }
        }
    }
...
/// entries, using the largest safe Leaf Entry batch for the current schema,
/// Backend budget, and configured split threshold, or at most
/// [`DRAIN_BATCH_INTERNAL`] Child Entries. The write
...
/// most one begin, one atomic expose-and-advance, one drain batch, or one
...
pub(crate) async fn advance_observed<B: Backend>(
    ...
    match state {
        ...
        PartitionTransition::Splitting { .. } => {
            ...
            .await?
            {
                TargetExposure::Exposed { left, right } => Ok(Advance::Exposed { left, right }),
