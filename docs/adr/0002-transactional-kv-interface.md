...
Ordered `scan` accepts a complete physical half-open byte range plus item and byte limits and returns ordered key/value items with an explicit terminal or non-terminal page state. A nonterminal page always contains at least one item, and its `next_start` is the byte-lexicographic successor of the page's last key — the smallest key strictly greater than it within the backend's key-length limit — which the page type derives rather than the adapter supplying an arbitrary cursor. A caller resumes by passing `next_start` back with the original end bound, so no key strictly between the last key and its successor is skipped and no returned key is duplicated. An empty nonterminal page is unrepresentable: the page constructor rejects it as a Backend error rather than a loop every caller must defend. This deliberately avoids a separate opaque transaction-bound continuation concept. Page byte accounting is exact encoded key plus value size. One item larger than the requested page byte limit may be returned alone, subject to backend hard limits, so pagination cannot make a valid value unreadable. Typed key codecs, not adapters, construct exact prefix ends. Core consumers request the next page only after confirming remaining search, transaction, or deletion capacity and perform no budget-external prefetch.
